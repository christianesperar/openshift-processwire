...
class Installer {
	...
	protected function dbConfig($values = array()) {
		...
		if(!isset($values['dbName'])) $values['dbName'] = getenv("OPENSHIFT_APP_NAME");
		// @todo: are there PDO equivalents for the ini_get()s below?
		if(!isset($values['dbHost'])) $values['dbHost'] = getenv("OPENSHIFT_MYSQL_DB_HOST"); 
		if(!isset($values['dbPort'])) $values['dbPort'] = getenv("OPENSHIFT_MYSQL_DB_PORT"); 
		if(!isset($values['dbUser'])) $values['dbUser'] = getenv("OPENSHIFT_MYSQL_DB_USERNAME"); 
		if(!isset($values['dbPass'])) $values['dbPass'] = getenv("OPENSHIFT_MYSQL_DB_PASSWORD");
		if(!isset($values['dbEngine'])) $values['dbEngine'] = 'MyISAM';

		if(!$values['dbHost']) $values['dbHost'] = getenv("OPENSHIFT_MYSQL_DB_HOST");
		if(!$values['dbPort']) $values['dbPort'] = getenv("OPENSHIFT_MYSQL_DB_PORT"); 
		...
		$this->input('dbPass', 'DB Pass', $values['dbPass'], false, getenv("OPENSHIFT_MYSQL_DB_PASSWORD"), false); 
